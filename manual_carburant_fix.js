...
async function manualCarburantFix() {
  try {
    console.log('🔧 Correction manuelle du problème de carburant...')
    ...
    console.log('\n📊 8. État final des données:')
    const finalStateResult = await query(`
      SELECT 
        COUNT(*) as total_carburant_records,
        COUNT(CASE WHEN employe_assigné IS NOT NULL THEN 1 END) as assigned_records,
        COUNT(CASE WHEN employe_assigné IS NULL THEN 1 END) as unassigned_records
    ...
    console.log(`Total records carburant: ${finalStateResult.rows[0].total_carburant_records}`)
    console.log(`Records avec employé: ${finalStateResult.rows[0].assigned_records}`)
    console.log(`Records sans employé: ${finalStateResult.rows[0].unassigned_records}`)
...
module.exports = { manualCarburantFix }
