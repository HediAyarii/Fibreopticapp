...
async function fixCarburantData() {
  try {
    console.log('🔧 Correction des données de carburant...')
    
    // 1. Vérifier et corriger les données de carburant sans employé assigné
    ...
    console.log('\n🔗 2. Liaison automatique des données:')
    
    // Méthode 1: Par nom dans les champs de carburant
    const autoLinkResult1 = await query(`
    ...
    console.log(`Records mis à jour par liaison automatique (méthode 1): ${autoLinkResult1.rowCount}`)
    
    // Méthode 2: Par numéro de carte
    const autoLinkResult2 = await query(`
      UPDATE carburant_consommation 
      SET employe_assigné = e.id
      FROM employes e
      WHERE carburant_consommation.employe_assigné IS NULL
    ...
    console.log(`Records mis à jour par liaison automatique (méthode 2): ${autoLinkResult2.rowCount}`)
    ...
    if (lotfiCheckResult.rows.length > 0) {
      ...
      const maiLotfiResult = await query(`
        ...
        WHERE employe_assigné = $1
          ...
          AND ca_ttc != '0'
          AND (
            cc.date_livraison LIKE '%05.2024%'
            OR cc.date_livraison LIKE '%05/2024%'
            OR cc.date_livraison LIKE '%2024-05%'
          )
      `, [lotfiId])
      ...
      WITH carburant_data AS (
        ...
        WHERE cc.date_livraison IS NOT NULL 
          ...
          AND (
            ...
            OR cc.date_livraison LIKE '%05/2024%'
            OR cc.date_livraison LIKE '%2024-05%'
            OR (cc.date_livraison ~ '^[0-9]{2}\.[0-9]{2}\.[0-9]{4}$' AND 
...
module.exports = { fixCarburantData }
