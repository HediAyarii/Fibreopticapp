-- Script pour créer la table primes_employes sur le serveur de production
-- Exécuter ce script sur la base de données PostgreSQL du serveur
...
CREATE TABLE IF NOT EXISTS primes_employes (
    ...
    note TEXT,
    deduit_rap BOOLEAN NOT NULL DEFAULT true,
...
-- Commentaires
COMMENT ON TABLE primes_employes IS 'Table des primes accordées aux employés';
COMMENT ON COLUMN primes_employes.cout_par_salaire_id IS 'Référence vers cout_par_salaire';
COMMENT ON COLUMN primes_employes.deduit_rap IS 'Si true, cette prime est déduite du reste à payer (RAP)';
