...
async function testFixedRecapCalcul() {
  try {
    console.log('🧪 Test de l\'API Récap Calcul corrigée...')
    ...
    // Test de la requête complète avec la correction
    ...
    console.log('📊 Résultats du test API corrigée:')
    console.log(`Nombre de techniciens trouvés: ${result.rows.length}`)
    
    // Rechercher spécifiquement LOTFI et BEN ABDALLAH
    ...
    const benAbdallahData = result.rows.find(row => 
      row.employe_nom && row.employe_prenom && 
      (row.employe_nom.toLowerCase().includes('ben') && 
       row.employe_prenom.toLowerCase().includes('abdallah'))
    )
    ...
    if (benAbdallahData) {
      console.log('\n🎯 Données trouvées pour BEN ABDALLAH:')
      console.log(`Nom: ${benAbdallahData.employe_nom} ${benAbdallahData.employe_prenom}`)
      console.log(`Matricule: ${benAbdallahData.matricule}`)
      console.log(`Interventions: ${benAbdallahData.nombre_interventions}`)
      console.log(`Recettes Technicien: ${Number(benAbdallahData.total_recette_technicien).toFixed(2)}€`)
      console.log(`Recettes Entreprise: ${Number(benAbdallahData.total_recette_entreprise).toFixed(2)}€`)
      console.log(`Transactions Carburant: ${benAbdallahData.nombre_transactions_carburant}`)
      console.log(`Consommation Carburant: ${Number(benAbdallahData.consommation_totale_carburant).toFixed(2)}€`)
      console.log(`Consommation Moyenne: ${Number(benAbdallahData.consommation_moyenne_carburant).toFixed(2)}€`)
    } else {
      console.log('\n❌ BEN ABDALLAH non trouvé dans les résultats')
    }
    ...
    console.log('\n✅ Test API corrigée terminé avec succès!')
    
  } catch (error) {
    console.error('❌ Erreur lors du test API corrigée:', error)
  }
}

// Exécuter le test si le script est appelé directement
if (require.main === module) {
  testFixedRecapCalcul()
}

module.exports = { testFixedRecapCalcul }
