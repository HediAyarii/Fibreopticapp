...
if result is not None:
    ...
    print("\n[v0] === SCHÉMA DE BASE DE DONNÉES PROPOSÉ ===")
    
    # Schéma pour les interventions (basé sur l'analyse du CSV)
    print("\nTable: interventions")
    if hasattr(result, 'columns'):
        for col in result.columns:
            print(f"  - {col}: TEXT")
    
    # Schéma pour le carburant (basé sur les attributs fournis)
    print("\nTable: carburant_consommation")
    carburant_cols = [
        "Date fact.", "Date de livraison", "Heure de livraison", "Immat. véhicule",
        "N° de carte", "km", "Poste 1", "Poste 2", "Pays", "N° de station",
        "Point d'acceptation", "Identifiant autoroute", "CP", "Type marchandises",
        "Quantité", "Taux TVA", "CA HT", "TVA", "CA TTC", "N° de justificatif"
    ]
