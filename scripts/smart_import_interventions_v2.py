...
def import_interventions_smart(csv_file_path):
    ...
    try:
        ...
        # cursor.execute("DELETE FROM interventions;")  # Commenté pour import intelligent
        conn.commit()
        
        # Préparer les données avec gestion intelligente des doublons
        interventions_data = []
        # Récupérer les interventions existantes pour comparaison intelligente
        try:
            existing_query = """
            SELECT num_inter, date_rdv, client, rue, 
                   type_intervention, statut, nom_technicien, 
                   date_creation, date_modification
            FROM interventions
            """
            existing_df = pd.read_sql(existing_query, conn)
            print(f"[INFO] Interventions existantes en base: {len(existing_df)}")
        ...
        for index, row in df.iterrows():
            try:
                ...
                if unique_key in seen_combinations:
                    duplicates_in_csv += 1
                    print(f"[DOUBLON] Doublon ignore: {num_inter} - {date_rdv}")
                    continue
                ...
                intervention_data = {
                    ...
                    'rue_pm': str(row.get('rue PM', '')).strip()
                }
        ...
        if interventions_data:
            insert_query = """
                INSERT INTO interventions (
                    date_rdv, region, plaque, societe, nom_nom_technicien, prenom_nom_technicien,
                    ...
                    echec_niveau_1, echec_niveau_2, panne_reseau, commentaires_nom_technicien,
