#!/usr/bin/env python3
"""
Script d'import de carburant avec gestion intelligente des doublons
"""
...
def import_carburant_smart(file_path, strategy='keep_last'):
    """
    Importer avec gestion intelligente des doublons
    
    Strategies:
    - 'keep_last': Garder le dernier enregistrement en cas de doublon
    - 'keep_first': Garder le premier enregistrement en cas de doublon
    - 'skip_all': Ignorer tous les doublons
    """
    print(f"🚀 Import carburant intelligent: {file_path}")
    print(f"📋 Stratégie de gestion des doublons: {strategy}")
    print("=" * 60)
    ...
    try:
        ...
        # Gérer les doublons selon la stratégie
        if strategy == 'keep_last':
            print(f"\n🔄 Suppression des doublons (garder le dernier)...")
            df_unique = df_mapped.drop_duplicates(subset=['numero_justificatif'], keep='last')
            print(f"   - Lignes après déduplication: {len(df_unique)}")
        elif strategy == 'keep_first':
            print(f"\n🔄 Suppression des doublons (garder le premier)...")
            df_unique = df_mapped.drop_duplicates(subset=['numero_justificatif'], keep='first')
            print(f"   - Lignes après déduplication: {len(df_unique)}")
        else:  # skip_all
            print(f"\n🔄 Suppression des doublons (ignorer tous)...")
            df_unique = df_mapped.drop_duplicates(subset=['numero_justificatif'], keep=False)
            print(f"   - Lignes après déduplication: {len(df_unique)}")
...
def main():
    parser = argparse.ArgumentParser(description='Import carburant avec gestion intelligente des doublons')
    parser.add_argument('--file', required=True, help='Chemin vers le fichier (CSV ou Excel)')
    parser.add_argument('--strategy', choices=['keep_last', 'keep_first', 'skip_all'], 
                       default='keep_last', help='Stratégie de gestion des doublons')
    ...
    success = import_carburant_smart(args.file, args.strategy)
