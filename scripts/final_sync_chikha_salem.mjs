...
async function finalSyncChikhaSalem() {
  try {
    console.log('🎯 Synchronisation finale pour CHIKHA SALEM...')
    ...
    // 5. Test de l'API cout-par-salaire
    ...
    if (apiTest.rows.length > 0) {
      ...
      console.log(`✅ API cout-par-salaire:`)
      console.log(`   - ${row.nom} ${row.prenom} (${row.matricule})`)
      console.log(`   - Total Généré: ${row.total_genere}€`)
      console.log(`   - RAP: ${row.rap}€`)
    }
    
    // 6. Test de l'API recap-calcul (Bénéfice Brut)
    console.log('\n🔍 Test de l\'API recap-calcul (Bénéfice Brut)...')
    const recapTest = await pool.query(`
      SELECT 
        SUM(
          CASE 
            WHEN i.statut = 'CLOTURE TERMINEE' THEN
      ...
      WHERE LOWER(i.nom_technicien) = LOWER('CHIKHA') AND LOWER(i.prenom_technicien) = LOWER('SALEM')
      AND (
        (i.cloture_tech >= '2025-05-01' OR i.cloture_hotline >= '2025-05-01') OR
        (i.cloture_tech IS NULL AND i.cloture_hotline IS NULL AND i.date_rdv >= '2025-05-01')
      )
      AND (
        (i.cloture_tech <= '2025-05-31' OR i.cloture_hotline <= '2025-05-31') OR
        (i.cloture_tech IS NULL AND i.cloture_hotline IS NULL AND i.date_rdv <= '2025-05-31')
      )
    `)
    
    const recapTotal = parseFloat(recapTest.rows[0].total_recettes) || 0
    console.log(`✅ API recap-calcul: ${recapTotal}€`)
    
    // 7. Comparaison finale
    console.log('\n📊 Comparaison finale:')
    console.log(`   - Base de données (cout_par_salaire): ${correctTotal}€`)
    console.log(`   - API recap-calcul (Bénéfice Brut): ${recapTotal}€`)
    
    const difference = Math.abs(correctTotal - recapTotal)
    if (difference < 0.01) {
      console.log('✅ PARFAIT ! Les deux sections sont maintenant cohérentes')
      console.log('✅ CHIKHA SALEM affiche 3770€ dans toutes les sections')
    } else {
      console.log(`❌ Il y a encore une différence de ${difference}€`)
    }
