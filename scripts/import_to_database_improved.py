#!/usr/bin/env python3
"""
Script d'importation amélioré des données CSV vers PostgreSQL
...
class DatabaseImporter:
    ...
    def find_column(self, df, possible_names):
        """Trouver une colonne par ses noms possibles"""
    ...
    def import_interventions(self, csv_file_path):
        ...
        try:
            ...
            for encoding in encodings:
                for sep in separators:
                    try:
                        ...
                        logger.info(f"✅ CSV parsé avec succès (encodage: '{encoding}', séparateur: '{sep}')")
                        break
                    except Exception as e:
                        logger.warning(f"⚠️ Échec avec encodage '{encoding}' et séparateur '{sep}': {e}")
                        continue
                if df is not None:
                    break
            ...
            # Trouver la colonne Num Inter avec différents noms possibles
            num_inter_col = self.find_column(df, [
                'Num Inter', 'num_inter', 'Numero Inter', 'numero_inter', 
                'Numero Intervention', 'numero_intervention', 'ID', 'id',
                'Intervention ID', 'intervention_id', 'Num', 'num'
            ])
            
            if not num_inter_col:
                logger.error("❌ Colonne 'Num Inter' non trouvée dans le CSV")
                logger.info("💡 Colonnes disponibles:")
                for col in df.columns:
                    logger.info(f"   - '{col}'")
                return False
            
            logger.info(f"🎯 Colonne 'Num Inter' trouvée: '{num_inter_col}'")
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    num_inter = str(row.get(num_inter_col, '')).strip()
                    if not num_inter or num_inter == 'nan' or num_inter == '':
                        empty_count += 1
                        continue
            ...
            logger.info(f"   - Lignes avec Num Inter vide: {empty_count}")
...
def main():
    parser = argparse.ArgumentParser(description='Import amélioré des données CSV vers PostgreSQL')
