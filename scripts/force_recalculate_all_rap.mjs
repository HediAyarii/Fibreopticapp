...
async function forceRecalculateAllRap() {
  console.log('🔧 Recalcul forcé de tous les RAP...');
  ...
  try {
    ...
    // 3. Recalculer chaque RAP avec la formule correcte
    console.log('\n📊 3. Recalcul de tous les RAP:');
...
forceRecalculateAllRap().catch(console.error);
