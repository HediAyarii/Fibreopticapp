...
// Instance Socket.IO globale - utiliser une propriété globale stable
...
function initializeSocketIO(server) {
  ...
  if (io) {
    console.log('✅ Socket.IO déjà initialisé, retour de l\'instance existante')
    return io
  }
  ...
  // Stocker l'instance globalement pour les routes API
  setIO(io);
  console.log('✅ Instance Socket.IO stockée dans global._socketIO')
...
function getSocketIOServer() {
  ...
  if (!socketIO) {
    console.warn('⚠️ Socket.IO non initialisé - global._socketIO est null')
  }
...
function sendReclamationTechniqueCreated(reclamationData) {
  const socketIO = getSocketIOServer()
  
  if (!socketIO) {
    console.warn('⚠️ Socket.IO non initialisé pour sendReclamationTechniqueCreated')
    return false
  }

  // Émettre à tous les admins connectés
  const clientsCount = socketIO.sockets.sockets.size
  console.log(`📡 Broadcasting reclamation_technique_created à ${clientsCount} client(s)`)
  socketIO.emit('reclamation_technique_created', reclamationData)
  console.log(`✅ Nouvelle réclamation technique émise: ${reclamationData.num_inter || reclamationData.id}`)
  return true
}
...
function sendReclamationTechniqueUpdated(reclamationData) {
  ...
  if (!socketIO) {
    console.warn('⚠️ Socket.IO non initialisé pour sendReclamationTechniqueUpdated')
    return false
  }

  // Émettre à tous les clients concernés
  const clientsCount = socketIO.sockets.sockets.size
  console.log(`📡 Broadcasting reclamation_technique_updated à ${clientsCount} client(s)`)
  socketIO.emit('reclamation_technique_updated', reclamationData)
